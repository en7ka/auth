...
// Address получаем хост и порт на котором запущен redis.
func (cfg *redisConfig) Address() string {
	return net.JoinHostPort(cfg.host, cfg.port)
}

// ConnectionTimeout получаем в connectionTimeout для redis.
func (cfg *redisConfig) ConnectionTimeout() time.Duration {
	return cfg.connectionTimeout
}

// MaxIdle получаем максимальное количество незанятых подключений.
func (cfg *redisConfig) MaxIdle() int {
	return cfg.maxIdle
}

// IdleTimeout получаем время закрытия простаивающих соединений.
func (cfg *redisConfig) IdleTimeout() time.Duration {
	return cfg.idleTimeout
}

// NewRedisConfig получаем переменные окружения для старта redis.
